...
/**
 * @package    BaseRefreshListActivity.kt
 * @author     luan
 * @date       2020-01-06
 * @des        提供刷新控件和列表控件的关联，以及处理列表分页
 *              该基类分页使用步骤：布局中添加recyclerView->调用ViewHelper.bind()方法进行绑定->获取到数据后调用notifyAdapter()
 */
abstract class BaseRefreshListActivity<P : BasePresenter<*>> : BaseRefreshActivity<P>(),
    OnItemClickListener {
    //list view的空布局
    ...
    override fun baseInit(savedInstanceState: Bundle?) {
        if (getListEmptyLayoutID() != 0) {
            listEmptyView = LayoutInflater.from(this).inflate(getListEmptyLayoutID(), null, false)
        }
    ...
    fun <B> RecyclerView.notifyAdapter(
        ...
        finishRefresh(this)
        //获取分页参数
        val pageParam = getTag(R.id.frame_tag_page) as? PageParam
        if (pageParam != null) {
...
//                    }
    ...
    open class PageParam {
        var currentPage = FrameConstants.Page.START_INDEX
        var pageCount = FrameConstants.Page.PAGE_COUNT
    }
